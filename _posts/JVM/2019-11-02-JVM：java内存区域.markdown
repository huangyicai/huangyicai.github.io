...
- ###  java虚拟机栈(Java Virtual Machine Stacks)
	...
	- 每个方法在执行的同时都会创建一个栈帧*（[栈帧结构详解](https://www.mdeditor.com/ "方法运行时的基础数据结构")）*用于存储局部变量、操作数栈、动态链接、方法出口等信息。每个方法从调用直至执行完成的过程，就对应着一个栈帧在虚拟机中入栈到出栈的过程。
	- 在JVM规范中，对这个区域规定了两种异常情况：*（[各内存区域的内存溢出异常情况详解](https://www.mdeditor.com/ "各个区域的内存溢出异常情况详解")）*
